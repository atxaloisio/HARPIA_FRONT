...
export class OmieService {
  ...
  getListfornecedoreservico(accessToken: string, id: number) {
    const listUrl = environment.urlbase + '/api/integracao';
    const headers = new Headers({
      Accept: 'application/json',
      Authorization: 'Bearer ' + accessToken.toString().replace(/"/g, '')
    });
    ...
    return this._http
      .get(listUrl + '/' + id.toString(), { headers: headers })
      .map((res: Response) => res)
      .catch((error: any) => Observable.throw(error.json() || 'Server error'));
  }

  syncProdutosERP(accessToken: string) {
    const listUrl = environment.urlbase + '/api/syncprodutos';
    const headers = new Headers({
      Accept: 'application/json',
      Authorization: 'Bearer ' + accessToken.toString().replace(/"/g, '')
    });
