 ...
 */
  ...
  excluirRegistro() {
    if (this.validaSelecao()) {
      this.dialog.question('SIH', 'Deseja realmente excluir o Centro de Custo: ' + this.selectedRow.id + '?').subscribe(
        result => {
          if (result.retorno) {
            this._centrocustoService.deleteCentroCusto(this._tokenManager.retrieve(), this.selectedRow.id).subscribe(
              data => {
                this.dialog.success('SIH', 'Centro de Custo excluído com sucesso.');
                this.ngOnInit();
              },
              error => {
