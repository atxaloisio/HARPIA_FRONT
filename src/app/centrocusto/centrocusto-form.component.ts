...
export class CentroCustoFormComponent
  ...
  validaCampos() {
    return this.valDescricao.valid;
  }
  ...
  ngOnInit() {
    ...
    this._omieService
      .getListCategorias(this._tokenManager.retrieve())
      .subscribe(data => {
        this.categorias = data.json();
      });

    this._omieService
      .getListContaCorrente(this._tokenManager.retrieve())
      .subscribe(data => {
    ...
    this._route.params.forEach((params: Params) => {
      ...
      if (id) {
        this._centrocustoService
          .getCentroCusto(this._tokenManager.retrieve(), id)
          .subscribe(data => {
  ...
  btnSalvar_click() {
    ...
    if (this.validaCampos()) {
      if (isNullOrUndefined(this.centrocusto.id)) {
        this._centrocustoService
          .addCentroCusto(this._tokenManager.retrieve(), this.centrocusto)
          .subscribe(
            data => {
              this.emProcessamento = false;
              ...
              this.dialog.success('SIH', 'Centro de Custo salvo com sucesso.');
            },
            error => {
              this.emProcessamento = false;
              this.dialog.error(
        ...
        this._centrocustoService
          .editCentroCusto(this._tokenManager.retrieve(), this.centrocusto)
          .subscribe(
            data => {
              this.emProcessamento = false;
              this.centrocusto = data;
  ...
  canDeactivate(): Observable<boolean> | boolean {
    if (
      JSON.stringify(this.centrocusto) === JSON.stringify(this.centrocusto_ant)
    ) {
      return true;
    }
