...
export class FuncaoFormComponent implements OnInit, AfterViewInit, AfterViewChecked {
  ...
  ngOnInit() {
    ...
    this._route.params.forEach((params: Params) => {
      ...
      if (id) {
        this._funcaoService.getFuncao(this._tokenManager.retrieve(), id).subscribe(data => {
  ...
  btnSalvar_click(form: NgForm) {
    ...
    if (this.validaCampos(form)) {
      if (isNullOrUndefined(this.funcao.id)) {
        this._funcaoService
          .addFuncao(this._tokenManager.retrieve(), this.funcao.descricao)
          .subscribe(
            data => {
              this.emProcessamento = false;
              this.funcao = data;
              ...
              this.dialog.success('SIH', 'Funcao salvo com sucesso.');
            },
            error => {
              this.emProcessamento = false;
              this.dialog.error(
        ...
        this._funcaoService
          .editFuncao(this._tokenManager.retrieve(), this.funcao.id, this.funcao.descricao)
          .subscribe(
            data => {
              this.emProcessamento = false;
              this.funcao = data;
