...
export class OrdemPagamentoService {
  ...
  addOrdemPagamento(
    accessToken: string,
    _ordempagamento: OrdemPagamento
  ): Observable<any> {
    let usuario: User;
  ...
  editOrdemPagamento(
    ...
    return this._http
      .put(
        this.ordempagamentoUrl + '/' + _ordempagamento.id.toString(),
        _body,
        { headers: headers }
      )
      .map((res: Response) => res.json())
  ...
  aprovarOrdemPagamento(
    accessToken: string,
    _ordempagamentos: OrdemPagamento[]
  ): Observable<any> {
    let usuario: User;
  ...
  deleteOrdemPagamento(accessToken: string, _id: number) {
    ...
    return this._http
      .delete(this.ordempagamentoUrl + '/' + _id.toString(), {
        headers: headers
      })
      .map((res: Response) => res.json())
      .catch((error: any) => Observable.throw(error.json() || 'Server error'));
  ...
  getOrdemPagamentos(
    ...
    if (
      !isNullOrUndefined(filter.centrocusto) &&
      filter.centrocusto.length > 0
    ) {
    ...
    if (
      !isNullOrUndefined(filter.contratante) &&
      filter.contratante.length > 0
    ) {
