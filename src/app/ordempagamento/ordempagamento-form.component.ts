...
export class OrdemPagamentoFormComponent
  ...
  ngOnInit() {
    ...
    this._centroCustoService
      .getListCentroCustoUsuario(this._tokenManager.retrieve(), usuario.id)
      .subscribe(data2 => {
    ...
    this._route.params.forEach((params: Params) => {
      ...
      if (id) {
        this._ordempagamentoService
          .getOrdemPagamento(this._tokenManager.retrieve(), id)
          .subscribe(data1 => {
  ...
  btnSalvar_click() {
    ...
    if (this.validaCampos()) {
      if (isNullOrUndefined(this.ordempagamento.id)) {
        this._ordempagamentoService
          .addOrdemPagamento(this._tokenManager.retrieve(), this.ordempagamento)
          .subscribe(
            data => {
              this.emProcessamento = false;
              ...
              this.dialog.success('SIH', 'OrdemPagamento salvo com sucesso.');
            },
            error => {
              this.emProcessamento = false;
              this.dialog.error(
  ...
  canDeactivate(): Observable<boolean> | boolean {
    if (
      JSON.stringify(this.ordempagamento) ===
      JSON.stringify(this.ordempagamento_ant)
    ) {
      return true;
    }
