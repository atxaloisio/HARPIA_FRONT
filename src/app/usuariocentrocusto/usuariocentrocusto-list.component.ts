...
export class UsuarioCentroCustoListComponent implements OnInit, AfterViewInit {
  ...
  excluirRegistro() {
    this.dialog
      .question(
        'SIH',
        'Deseja realmente remover as associações de centro de custo com o usuario selecionado?'
      )
      .subscribe(result => {
        if (result.retorno) {
          this._usuariocentrocustoService
            .deleteUsuarioCentroCusto(
              this._tokenManager.retrieve(),
              this.usuarioFilter.value
            )
            .subscribe(
              data => {
                this.dialog.success(
                  'SIH',
                  'Associações de centro de custo removidas com sucesso.'
                );
                this.ngOnInit();
              },
              error => {
      ...
      this._usuariocentrocustoService
        ...
        .subscribe(
          data => {
            this.dialog.success(
              'SIH',
              'Centro de custo associado ao usuário com sucesso.'
            );
  ...
  ngOnInit() {
    ...
    this._userService
      .getListUsers(this._tokenManager.retrieve())
      .subscribe(data => {
    ...
    this.dataSource = new DsUsuarioCentroCusto(
      this._tokenManager,
      this._usuariocentrocustoService,
      this.paginator,
      this.sort
    ...
    const descricaoFilter$ = this.descricaoFilter.valueChanges
      ...
      .distinctUntilChanged()
      .startWith('');
    const usuarioFilters$ = this.usuarioFilter.valueChanges
      .debounceTime(500)
      .distinctUntilChanged()
      .startWith('');

    combineLatest(idFilter$, descricaoFilter$, usuarioFilters$)
      .debounceTime(500)
      .distinctUntilChanged()
      .map(([id, descricao, id_usuario]) => ({ id, descricao, id_usuario }))
      .subscribe(filter => {
        if (!this.dataSource) {
          return;
        }
