...
export class LoginComponent implements OnInit {
  ...
  ngOnInit() {
    this._omieService.getListEmpresas().subscribe(data => {
  ...
  onEntrarClick(): void {
    ...
    if (
      !isNullOrUndefined(this.data.email) &&
      !isNullOrUndefined(this.data.password)
    ) {
      this.loginService
        .Login(this.data.email, this.data.password, this.data.id_empresa)
        .subscribe(
          data => {
            // console.log(data);
            this.data.Usuario = data.usuario;
            this.data.Perfil = data.perfil;
            this.data.Empresa = data.empresa;
            ...
            if (data.aprov_pendentes > 0) {
              if (data.aprov_pendentes > 1) {
                this.openSnackBar(
                  'Você possui ' +
                    data.aprov_pendentes +
                    ' ordens de pagamento pendentes para aprovação',
                  'SIH'
                );
              } else {
                this.openSnackBar(
                  'Você possui ' +
                    data.aprov_pendentes +
                    ' ordem de pagamento pendente para aprovação',
                  'SIH'
                );
              }
            }
