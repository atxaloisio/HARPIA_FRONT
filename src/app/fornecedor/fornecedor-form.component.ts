...
export class FornecedorFormComponent
  implements OnInit, AfterViewInit, AfterViewChecked, CanDeactivate<NgForm> {
  fornecedor: Fornecedor;
  fornecedor_ant: Fornecedor;

  linkDownload =
    environment.urlbase + '/api/documentos/fornecedor/downloadanexo?arquivo=';
  ...
  validaCampos() {
    return this.valRazaoSocial.valid && this.valCnpjCpf.valid;
  }
  ...
  ngOnInit() {
    ...
    this._funcaoService
      .getListFuncao(this._tokenManager.retrieve())
      .subscribe(data => {
        ...
        .subscribe(data => {
          ...
          if (listfornecedor.length > 0) {
            if (listfornecedor[0].id !== this.fornecedor.id) {
              this.dialog.warning(
                'SIH',
                'CNPJ / CPF informado já se encontra cadastrado no sistema.'
              );
              this.fornecedor.cnpj_cpf = '';
            }
          }
        ...
        this._fornecedorService
          ...
          .subscribe(
            data => {
              ...
              this.dialog.success('SIH', 'Fornecedor salvo com sucesso.');
            },
            error => {
              this.emProcessamento = false;
              this.dialog.error(
  ...
  getErrorMessage(control: FormControl) {
    ...
    if (control.hasError('date')) {
      ...
      mensagem =
        mensagem + 'Data informada inferior a ' + data.toLocaleDateString();
    }
  ...
  canDeactivate(): Observable<boolean> | boolean {
    if (
      JSON.stringify(this.fornecedor) === JSON.stringify(this.fornecedor_ant)
    ) {
      return true;
    }

    return this.dialog.confirm(
      'Existem dados não salvos. Deseja descarta-los?'
    );
  }
}
