...
export class FornecedorListComponent implements OnInit, AfterViewInit {
  ...
  dblck(row) {
    this._router.navigate(['/prestadores/prestador', { id: row.id }]);
  }
  ...
  incluirRegistro() {
    this._router.navigate(['/prestadores/prestador']);
  }

  editarRegistro() {
    if (this.validaSelecao()) {
      this._router.navigate(['/prestadores/prestador', { id: this.selectedRow.id }]);
      this.ngOnInit();
      this.selectedRowIndex = -1;
      this.selectedRow = null;
    }
  ...
  excluirRegistro() {
    if (this.validaSelecao()) {
      this.dialog
        .question('SIH', 'Deseja realmente excluir o fornecedor: ' + this.selectedRow.id + '?')
        .subscribe(result => {
          if (result.retorno) {
            this._fornecedorService
              .deleteFornecedor(this._tokenManager.retrieve(), this.selectedRow.id)
              .subscribe(
                data => {
                  this.dialog.success('SIH', 'Fornecedor excluído com sucesso.');
                  this.ngOnInit();
                },
                error => {
                  this.dialog.error(
  ...
  ngOnInit() {
    ...
    this.dataSource = new DsFornecedor(
      this._tokenManager,
      this._fornecedorService,
      this.paginator,
      this.sort
    ...
    const idFilter$ = this.idFilter.valueChanges
      ...
      .startWith('');
    const codigoFilter$ = this.codigoFilter.valueChanges
      .debounceTime(500)
      .distinctUntilChanged()
      .startWith('');
    const cnpjCpfFilter$ = this.cnpjCpfFilter.valueChanges
      .debounceTime(500)
      .distinctUntilChanged()
      .startWith('');
    const razaoSocialFilter$ = this.razaoSocialFilter.valueChanges
      ...
      .distinctUntilChanged()
      .startWith('');
    const contatoFilter$ = this.contatoFilter.valueChanges
      .debounceTime(500)
      .distinctUntilChanged()
      .startWith('');
    const telefoneFilter$ = this.telefoneFilter.valueChanges
      .debounceTime(500)
      .distinctUntilChanged()
      .startWith('');
    const emailFilter$ = this.emailFilter.valueChanges
      ...
      .distinctUntilChanged()
      .startWith('');
    const inativoFilter$ = this.inativoFilter.valueChanges
      .debounceTime(500)
      .distinctUntilChanged()
      .startWith('');
    ...
    combineLatest(
      ...
      .debounceTime(500)
      .distinctUntilChanged()
      .map(([id, codigo_omie, cnpj_cpf, razao_social, contato, telefone, email, inativo]) => ({
        id,
        codigo_omie,
        cnpj_cpf,
        razao_social,
